        ...
        <div className="text-center py-3">
          {orders?.map((item, i) => (
            <Card className="CardAddress" key={i} id={item.orderCreationId}>
              <h5 className="AddrLineText">{item.orderCreationId.toUpperCase()}</h5>
              <hr />
              ...
              <Card className="CardAddress">
                ...
                <hr />
                <p className="AddrLineText">
                  <b>{item.address.firstName + ' ' + item.address.lastName}</b>
                </p>
                <p className="AddrLineText">Mobile No :{item.address.mobileno}</p>
                <p className="AddrLineText">
                  {item.address.atype !== undefined ? item.address.atype : null}
                </p>
                <p className="AddrLineText">{item.address.address}</p>
                <p className="AddrLineText">
                  {item.address.landmark !== undefined ? item.address.landmark : null}
                </p>
                <p className="AddrLineText">{item.address.area}</p>
                <p className="AddrLineText">{item.address.city + ' - ' + item.address.pincode}</p>
                <p className="AddrLineText">{item.address.state + ', ' + item.address.country}</p>
              </Card>
              <Button
                color="danger"
                type="button"
                onClick={() =>
                  sendPDF(document.querySelector('#' + item.orderCreationId).innerHTML)
                }>
                PDF
              </Button>
            </Card>
...
const sendPDF = (htmltxt) => {
  ...
  axios.post(baseURL + 'sendPDF', { html: htmltxt }).then(
    (response) => {
      const win = window.open(baseURL + 'sendPDF/file?file=' + response.data.filename, '_blank');
      win.focus();
    },
    (error) => {
      console.log(error);
